...
export async function initCoverage() {
  ...
  const [membres, teams, slots] = await Promise.all([
    safeQuery('coverage:membres', supabase.from('membres').select('id,nom,spe,classe,rio,ilvl,cle_donjon,cle_niveau,can_trade,discord_tag,main_id')),
...
export async function refreshCoverage() {
  ...
  const membres = await safeQuery('coverage:refresh',
    supabase.from('membres').select('id,nom,spe,classe,rio,ilvl,cle_donjon,cle_niveau,can_trade,discord_tag,main_id')
  );
  if (!membres) return;
